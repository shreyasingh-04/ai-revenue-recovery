...
function StatCard({ title, value, icon: Icon, colorClass, gradientClass, delay, onClick, isActive }) {
  return (
    <motion.div 
      ...
      transition={{ delay }}
      onClick={onClick}
      className={`relative overflow-hidden bg-white/5 backdrop-blur-xl border ${isActive ? 'border-white/40 ring-1 ring-white/20 bg-white/10' : 'border-white/10'} rounded-3xl p-6 transition-all duration-300 hover:bg-white/10 hover:border-white/20 group cursor-pointer`}
    >
...
export default function Dashboard({ metrics, orders }) {
  ...
  const filteredOrders = orders.filter(o => {
    ...
    if (filter === 'recovered') return o.status === 'recovered';
    if (filter === 'unrecovered') return o.status === 'unrecovered';
    return true;
  });
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold tracking-tight text-white">
              {filter === 'all' ? 'All Orders' : filter === 'recovered' ? 'Recovered Orders' : 'Unrecovered Orders'}
            </h2>
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            <AnimatePresence>
              {filteredOrders.slice(0, 50).map((order, index) => (
                <motion.div 
                  ...
                  transition={{ delay: index < 10 ? 0.1 + (index * 0.05) : 0 }}
                  key={order.id} 
            ...
            {filteredOrders.length === 0 && (
              <div className="text-slate-400 text-sm text-center mt-12">No orders match this filter.</div>
            )}
          </div>
        </motion.div>
      </div>
