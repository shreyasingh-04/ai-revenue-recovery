...
function StatCard({ title, value, icon: Icon, colorClass, gradientClass }) {
  return (
    <div className={`relative overflow-hidden bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 transition-all duration-300 hover:bg-white/10 hover:border-white/20 group`}>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="relative z-10 h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 30 }}>
                ...
                />
                <Bar dataKey="rate" radius={[8, 8, 8, 8]} maxBarSize={50}>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 flex flex-col h-[450px]">
          <h2 className="text-xl font-semibold tracking-tight text-white mb-6">Recent Drop-offs</h2>
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {orders.slice(0, 10).map(order => (
              <div key={order.id} className="p-4 bg-white/5 hover:bg-white/10 transition-colors rounded-2xl border border-white/5 flex items-center justify-between">
                <div>
